...
router.post('/createUser', [
    ...
    try {
        ...
        if (user) return res.status(400).json({ success, msg: 'Sorry Email 📩 already exists' })
...
router.post('/loginUser', [
    ...
    try {
        ...
        if (!user) return res.status(400).json({ success, msg: 'Please try to login with correct credential' });

        const pwdCompare = await bcrypt.compare(pwd, user.pwd)
        if (!pwdCompare) return res.status(400).json({ success, msg: 'Please try to login with correct credential' });
